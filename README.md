...
- find and replace `ajb-next-start`
- find and replace `AJB Next Starter`
- If you don't have the [Sanity CLI](https://www.sanity.io/docs/getting-started-with-sanity-cli) installed, first run `npm install -g @sanity/cli` to install it globally
- `npm install && sanity init` in the `/sanity` folder
- During Sanity's initalization it will warn you, type `Y` and hit `enter`:
